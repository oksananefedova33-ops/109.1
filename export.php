...
// Генерируем файлы аналитики
        generateAnalyticsFiles($exportDir);
...
HTML;
  ...
  $html .= <<<HTML
    ...
    <!-- Analytics Tracker -->
    <script src="{$assetsPath}/js/tracker.js?v={$buildVersion}" defer></script>
...
// Генерация файлов аналитики
function generateAnalyticsFiles($exportDir) {
    // 1. Создаем tracker.js
    $trackerJsTemplate = __DIR__ . '/ui/analytics/tracker-template.js';
    if (file_exists($trackerJsTemplate)) {
        $trackerJs = file_get_contents($trackerJsTemplate);
        file_put_contents($exportDir . '/assets/js/tracker.js', $trackerJs);
    }
    
    // 2. Создаем analytics-tracker.php
    $trackerPhpTemplate = __DIR__ . '/ui/analytics/analytics-tracker-template.php';
    if (file_exists($trackerPhpTemplate)) {
        $trackerPhp = file_get_contents($trackerPhpTemplate);
        file_put_contents($exportDir . '/analytics-tracker.php', $trackerPhp);
    }
    
    // 3. Создаем analytics-api.php
    $apiPhpTemplate = __DIR__ . '/ui/analytics/analytics-api-template.php';
    if (file_exists($apiPhpTemplate)) {
        $apiPhp = file_get_contents($apiPhpTemplate);
        file_put_contents($exportDir . '/analytics-api.php', $apiPhp);
    }
}